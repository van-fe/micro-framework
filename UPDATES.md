...
记录各版本最终保留的新增能力、修复和行为变化；同一版本内合并更新，不按任务重复追加。当前版本：**0.1.0**。

## 0.1.0 — 2026-09-15

### 发布流程

- 根项目、框架库、文档与示例工作区统一为 `0.1.0`，内部 workspace 依赖在 npm 暂存产物中固定为同批版本。
- npm Action 分离无发布凭据的检查/打包与 `npm` environment 中的实际发布；GitHub Release 发布后按依赖顺序发布 public `@micro-framework/*` 包，手动运行默认 dry-run，未填写 tag 时可验证所选分支的当前版本。
- 检查前先构建框架，确保全新 CI checkout 中 Angular 示例可以解析适配器的声明产物，不依赖本地遗留 `dist`。
- 实际发布必须指定与根版本匹配的已有 tag；保留整批完整性检查、相同版本重试保护，以及 Node 24 的 OIDC / `NPM_TOKEN` 发布支持。体积检查失败也上传诊断报告，不跳过现有发布门禁。
- 验证：冻结安装、架构检查、完整构建（含中英文文档与示例）、类型检查、192 项单元测试、8 项发布脚本测试及 Action 语法检查通过；单独诊断打包与离线 dry-run 验证了 30 个 `0.1.0` npm 包。Runtime gzip 仍为 60,383 B，超过 50,000 B 门禁，正式发布仍被阻止；尚未发布 npm。
...
- 文档站提供完整中英双语页面：保留中文根路径，英文使用 `/en/`，导航、搜索、页脚和同篇语言切换均按语言配置；共享演示组件按文档语言初始化宿主与四个微应用。
...
- 双语文档 38 对页面构建通过；三浏览器独立/中英文内嵌演示、桌面与移动语言切换及本地分语言搜索 15/15 通过，宿主和测试类型检查通过；线上搜索验收等待 VitePress 完成异步页面加载与 hydration 后再交互，避免把 SSR 正文可见误判为控件已就绪。
- Pages 完整站点构建及三浏览器独立/内嵌演示初始 6/6 通过，覆盖四框架真实弹层、市场/语言切换、隔离和销毁；最终产物的 HTML/JS/CSS 不含开发服务器地址。冻结安装、全项目类型检查、架构检查、192 项单测与工作流 actionlint 通过。
